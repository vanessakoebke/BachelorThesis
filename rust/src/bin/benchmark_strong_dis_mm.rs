...
use strongdis_runtime::strong_dis::strong_dis_mm;
...
fn main() -> io::Result<()> {
    ...
    for file in files {
        ...
        strong_dis_mm(&instance.matrix, instance.n, instance.a, instance.b);

        let start = Instant::now();
        let result = strong_dis_mm(&instance.matrix, instance.n, instance.a, instance.b);
        let elapsed = start.elapsed().as_nanos();

        rows.push((file.display().to_string(), elapsed, result));
    }

    let mut output = File::create(results_dir.join("rust_results_MM.csv"))?;
